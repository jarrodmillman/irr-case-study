...
for i in range(len(x)):
#for i in range(4):  # loop over categories
    print i+1
...
nsgk_res = np.concatenate((np.array(range(len(x)+1))[1:][:,None], category_pvalues[:,None], video_pval, video_conc, video_means, video_sd), axis = 1)
