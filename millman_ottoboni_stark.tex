...
In the context of this project, \emph{reproducibility} means that
...
We've also recorded our code review discussions along with the code produced
as a result of those discussion.
...
We also have written tests for (almost) all our code, which means we have a
high level of confidence that as we make changes to our codebase we can catch
any cascading errors that result.
Catching errors early means we can more quickly and easily identify our
mistakes and correct them.
And since we've automated the process of running our analysis, once errors
are identified and corrected, it is trivial to rerun the entire analysis from
start to finish.

Assuming you have network access and that you have Bash, Python, Git, and Make
installed on your computer, you can run our complete analysis of the cleaned
data by typing the following three commands:
\begin{verbatim}
$ git clone git@github.com:statlab/nsgk.git
$ cd nsgk/nsgk
$ make
\end{verbatim}
We will discuss these tools in \S~\ref{key-tools} below.
However, let's quickly walk through what happens when you enter the above
commands.
The first command will create a directory named \texttt{nsgk} in your current
...
analysis.
%Our Python script (\texttt{analysis.py}) is in a subdirectory \texttt{nsgk} of
%\texttt{nsgk}.
Inside of \texttt{nsgk/nsgk} there is a \texttt{Makefile}, our analysis script
\texttt{analysis.py}, and \texttt{results.csv} the output of our analysis.

When you enter the command \texttt{make}, the \texttt{Makefile} recipe
corresponding to the default target \texttt{all} will be invoked.
\begin{verbatim}
all:
...
clean:
        rm -rf venv
\end{verbatim}
The default recipe (\texttt{all}) creates a new virtual environment (see
\S~\ref{key-tools}) for Python 2.7, upgrades the Python package manager
(\texttt{pip}), installs the necessary Python package dependencies
(\texttt{numpy}, \texttt{scipy}, and \texttt{permute}), and runs the
...
project.\footnote{Exceptions including that we didn't record all of our
...
working with code, data, or text on a computer, this aspect of the project was
...
(2) the process is largely self-documenting, making it easier to draft a
    paper about the results; and
...
across across raters and across strata.
